...
impl StartWorkflowOptions {
    ...
    /// # Examples
    /// ```
    /// use flovyn_worker_sdk::client::StartWorkflowOptions;
    ///
    ...
    pub async fn start(&self) -> Result<WorkerHandle> {
        ...
        let (task_handle, task_ready, task_running) = if self.task_registry.has_registrations() {
            let config = TaskWorkerConfig {
                ...
                poll_timeout: self.poll_timeout,
                no_work_backoff: Duration::from_millis(100),
                max_concurrent: self.config.task_config.max_concurrent,
        ...
        let (agent_handle, agent_ready, agent_running) = if self.agent_registry.has_registrations()
        {
            let config = AgentWorkerConfig {
                ...
                org_id: self.org_id,
                queue: self.queue.clone(),
                poll_timeout: self.config.agent_config.poll_timeout,
                no_work_backoff: Duration::from_millis(100),
                max_concurrent: self.config.agent_config.max_concurrent,
                heartbeat_interval: self.config.agent_config.heartbeat_interval,
                worker_name: self.worker_name.clone(),
                worker_version: self.worker_version.clone(),
                team_id: self.team_id,
                enable_auto_registration: false, // Unified registration already done
