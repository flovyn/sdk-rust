...
mod tests {
    ...
    fn test_replay_event_new() {
        let event = ReplayEvent::new(
            ...
            json!({"operationName": "fetch-user", "result": {"id": 123}}),
            now(),
        );
    ...
    fn test_replay_event_serde() {
        let event = ReplayEvent::new(
            1,
            EventType::OperationCompleted,
            json!({"operationName": "test", "result": 42}),
            now(),
        );
    ...
    fn test_replay_event_builder_operation_name() {
        let event = ReplayEvent::new(0, EventType::OperationCompleted, json!({}), now())
    ...
    fn test_replay_event_builder_result() {
        let event = ReplayEvent::new(0, EventType::OperationCompleted, json!({}), now())
            .with_result(json!({"id": 123}));

        assert_eq!(event.get("result"), Some(&json!({"id": 123})));
    }

    #[test]
    fn test_replay_event_builder_state_key() {
        let event = ReplayEvent::new(0, EventType::StateSet, json!({}), now())
    ...
    fn test_replay_event_builder_task_type() {
        let event = ReplayEvent::new(0, EventType::TaskScheduled, json!({}), now())
    ...
    fn test_replay_event_builder_timer_id() {
        let event = ReplayEvent::new(0, EventType::TimerStarted, json!({}), now())
    ...
    fn test_replay_event_builder_promise_name() {
        let event = ReplayEvent::new(0, EventType::PromiseCreated, json!({}), now())
