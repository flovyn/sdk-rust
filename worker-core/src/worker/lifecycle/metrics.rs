...
pub struct WorkerMetrics {
    ...
    /// Time since worker started (milliseconds).
    pub uptime_ms: u64,

    // Internal tracking for average calculation (not exposed via serde)
    #[serde(skip)]
...
impl WorkerMetrics {
    ...
    pub fn record_workflow_completed(&mut self, duration_ms: u64) {
    ...
    pub fn record_workflow_failed(&mut self, duration_ms: u64) {
        if self.workflows_in_progress > 0 {
            self.workflows_in_progress -= 1;
        }
    ...
    pub fn record_task_completed(&mut self, duration_ms: u64) {
    ...
    pub fn record_task_failed(&mut self, duration_ms: u64) {
        if self.tasks_in_progress > 0 {
            self.tasks_in_progress -= 1;
        }
        self.task_errors += 1;
...
mod tests {
    ...
    fn test_record_workflow_completed() {
        ...
        metrics.record_workflow_completed(100);
    ...
    fn test_record_workflow_failed() {
        let mut metrics = WorkerMetrics::new();
        metrics.record_workflow_started();
        metrics.record_workflow_failed(50);
    ...
    fn test_average_calculation() {
        ...
        metrics.record_workflow_started();
        metrics.record_workflow_completed(200);

        metrics.record_workflow_started();
        metrics.record_workflow_completed(300);
    ...
    fn test_total_errors() {
        let mut metrics = WorkerMetrics::new();
        metrics.record_workflow_started();
        metrics.record_workflow_failed(10);
        metrics.record_task_started();
        metrics.record_task_failed(10);
    ...
    fn test_total_executions() {
        let mut metrics = WorkerMetrics::new();
        metrics.record_workflow_started();
        metrics.record_workflow_completed(10);
        metrics.record_task_started();
        metrics.record_task_completed(10);
    ...
    fn test_update_uptime() {
        let mut metrics = WorkerMetrics::new();
        metrics.update_uptime(60000);
        assert_eq!(metrics.uptime_ms, 60000);
    }
    ...
    fn test_metrics_serde() {
        ...
        assert_eq!(parsed.workflows_executed, 1);
        assert_eq!(parsed.avg_workflow_duration_ms, 100.0);
    }
}
